...
function OZWDevice(ozw, nodeid) {
  ...
  // assume true for framework to receive state updates
  this.connected = true;
}

util.inherits(OZWDevice, CdifDevice);

// deliberately do not implement _connect and _disconnect
// so we assume the device always stays in connected state
...
OZWDevice.prototype.notifyValueUpdate = function(comClass, value) {
  ...
  this.setServiceStates(serviceID, val, function(err) {
    if (err) {
      console.error('ozw device send status update failed:' + err);
    }
  });
};
